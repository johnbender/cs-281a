  ...
  \item Let $f : \mathbb{N} \rightarrow \mathbb{N}$ and $g : \mathbb{N} \rightarrow \mathbb{N}$ be time-constructible. Prove that the following functions are also time-constructible: (a) $f + g$; (b) $f \cdot g$.
    ...
    \begin{proof} (a)
      ...
      \begin{itemize}
        ...
        \item Either $f = O(g)$, $g = O(f)$ or $f = \Theta(g)$ and for sufficiently large $|x|$. That is $O(f(n) + g(n)) = \Theta(f(n))$ or alternately $O(f(n) + g(n)) = \Theta(g(n))$
      \end{itemize}

      As a result we know that a TM computing $h(n)$ will be $\Theta(f(n))$ or $\Theta(g(n))$, in other words $M_h$ can safely emulate both $M_f$ and $M_g$ and remain $O(f(n) + g(n))$.

      \textbf{NOTE} It seems like $+$ should be accounted for here as well since its runtime should be at least $|f(n)| + |g(n)|$ and both $f$ and $g$ might be runtime $n$ according to the definition in the book.
    \end{proof}
    ...
    \begin{proof} (b)
      ...
      Similar to addition, the actual computation will be bounded by either $f$ or $g$ so it is certainly the case that the runtime of a Turing machine for $f(n) \cdot g(n)$ be $O(f(n) \cdot g(n))$.
